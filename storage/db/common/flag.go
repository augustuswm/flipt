...
func (s *Store) GetFlags(ctx context.Context, keys []string) ([]*flipt.Flag, error) {
	query := s.builder.Select("\"key\", name, description, enabled, created_at, updated_at").
		From("flags").
...
func (s *Store) processFlagListQuery(ctx context.Context, query sq.SelectBuilder) ([]*flipt.Flag, error) {
